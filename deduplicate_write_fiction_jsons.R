...
#load anachronisms, contains authors which don't write fiction or write books for children
anachronisms <- readLines("removal_authors.txt")

#load and clean data
data <- read.csv("multiHATHI.csv", stringsAsFactors = FALSE) %>%
  as_tibble() %>%
  filter(fic == 1, lang == "eng") %>%
  mutate(
    ...
    author = gsub("from old catalog", "", author),
    author = gsub("old catalog heading", "", author),
    #apply corrections in a single step using case_when
    author = case_when(
...
#standardize filenames in htid for `data`
data <- data %>%
  mutate(
...
data <- bind_rows(data_failure, data_success) %>%
  ...
  filter(year >= 1800 & year <= 2010)

#change this column name
colnames(data)[2] <- "pub_date"

data <- data %>% arrange(pub_date)

#write to CSV
write.csv(data, "fiction_jsons.csv", row.names = FALSE)